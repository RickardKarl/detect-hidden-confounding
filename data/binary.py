...
class BinaryCauseData(AbstractData):
    ...
    def __init__(self, nbr_env: int, dist_param: dict, seed=None) -> None:
        ...
        for m in ['T', 'X', 'Y']:
            for p in BinaryCauseData.required_dist_param:
    ...
    def sample(self, nbr_samples: int, conf_strength: float) -> dict:
        ...
        for i in range(self.nbr_env):
            ...
            prob_Te = sigmoid(conf_strength * x_alpha * Xe + t_mu)
            Te = self.rng.binomial(1, prob_Te) 

            prob_Ye =     sigmoid(conf_strength * x_beta * Xe + t_beta * Te + y_mu)
