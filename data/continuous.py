...
class GaussianData(BinaryCauseData):
    ...
    def generate_params(self) -> dict:
        ...
        var_mech['X_sigma'] = self.dist_param['X_sigma'] if 'X_sigma' in self.dist_param else 5  # noise variance for X (intentionally 2) 
        var_mech['Y_sigma'] = self.dist_param['Y_sigma'] if 'Y_sigma' in self.dist_param else 1  # noise variance for X 
        var_mech['T_sigma'] = self.dist_param['T_sigma'] if 'T_sigma' in self.dist_param else 1  # noise variance for T 

        return var_mech

    def sample(self, nbr_samples: int, conf_strength: float) -> dict:
        '''
        ...
        for i in range(self.nbr_env):
            ...
            # Retrieve parameters
            x_mu = np.abs(self.params['X_mu'][i])
            t_mu = np.abs(self.params['T_mu'][i])
            y_mu = np.abs(self.params['Y_mu'][i])
            ...
            Te = conf_strength * x_alpha * Xe + e_T  

            e_Y = self.rng.normal(y_mu, y_sigma, size=(nbr_samples))
            Ye = conf_strength * x_beta * Xe + t_beta * Te + e_Y
